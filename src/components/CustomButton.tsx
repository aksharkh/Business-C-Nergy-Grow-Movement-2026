...
const CustomButton = ({ children, primary = false, href = '#' }: ButtonProps) => (
  <a
    href={href}
    className={`
      group relative inline-flex items-center justify-center px-8 py-4 text-sm font-bold tracking-[0.2em] uppercase overflow-hidden transition-all duration-500 shadow-lg hover:shadow-blue-500/40 rounded-sm
      ${
        primary
          ? 'bg-[#0F52BA] text-white hover:text-white'
